...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (countdownEl) {
    const eventDate = new Date('2026-04-12T19:00:00').getTime();
  ...
  if (surpriseBtn && eventCards.length) {
    surpriseBtn.addEventListener('click', () => {
      eventCards.forEach(card => card.classList.remove('highlight'));
      const randomIndex = Math.floor(Math.random() * eventCards.length);
      eventCards[randomIndex].classList.add('highlight');
      setTimeout(() => {
        eventCards[randomIndex].classList.remove('highlight');
      }, 3000);
    });
  }
});
